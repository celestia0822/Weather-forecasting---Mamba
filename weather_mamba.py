...
df = pd.read_csv('weather.csv')#调取文件及路径

# 特征工程
features1 = ['temp', 'pressure', 'wind_speed']
#df['dt'] = pd.to_datetime(df['dt'], unit='s')
#df.sort_values('dt', inplace=True)

df['dt'] = [datetime.datetime.utcfromtimestamp(x) for x in df['dt']]
# Sorting by the date 
df.sort_values('dt', inplace=True)
df = df.groupby('dt', as_index=False)[features1].mean()
...
s = 24 * 60 * 60
year = (365.25) * s
#df['month_cos'] = np.cos((df['dt'].astype('int64').astype(int) // 10**9) * (2 * np.pi / year))
#df['month_sin'] = np.sin((df['dt'].astype('int64').astype(int) // 10**9) * (2 * np.pi / year))
...
parser.add_argument('--layer', type=int, default=2,
                    help='Num of layers')
parser.add_argument('--n-test', type=int, default=103290,
...
# 特征提取&归一化处理
features=['temp','day_cos', 'day_sin', 'month_sin', 'month_cos','pressure', 'wind_speed']
feature_data = df[features].values.reshape(-1,7)
...
# 将归一化后的特征数据放回原来的 DataFrame 中
df[features] = scaled_feature_data
df['temp']=scaled_temp

# 拆分数据集
train_df, test_df = df.iloc[:-args.n_test], df.iloc[-args.n_test:]
train_X, train_y = train_df[features].values, train_df['temp'].values
test_X, test_y = test_df[features].values, test_df['temp'].values
train_df.dtypes
...
def PredictWithData(trainX, trainy, testX):
    ...
    for e in range(args.epochs):
        ...
        if e%10 == 0 and e!=0:
            ...
            print('Epoch %d | Duration: %.2f seconds | Loss: %.4f' % (e + 1, duration, loss.item()))
...
'''def train_model(model, train_X, train_y,epochs,lr,wd):
    criterion = nn.MSELoss()
    model = Net(len(trainX[0]),1)
    optimizer = torch.optim.Adam(model.parameters(),lr=args.lr,wd=args.wd)
    xt = torch.from_numpy(train_X).float().unsqueeze(0)
    xv = torch.from_numpy(test_X).float().unsqueeze(0)
    
    for epoch in range(args.epochs):
        model.train()
        optimizer.zero_grad()
        output = model(torch.Tensor(train_X))
        #loss = criterion(output, torch.Tensor(train_y).unsqueeze(0))
        loss.backward()
        optimizer.step()
        if epoch % 10 == 0:
...
'''def predict(model, test_X):
    model.eval()
    with torch.no_grad():
        predictions = model(torch.Tensor(test_X))
...
# 训练模型
#model = Net(len(train_X[0]), 1)
predictions=PredictWithData(train_X,train_y,test_X)
...
plt.plot(test_df['dt'], test_y, label='Actual Temperature')
plt.plot(test_df['dt'], predictions, label='Predicted Temperature')
